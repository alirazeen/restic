...
func saveFile(t testing.TB, repo restic.Repository, filename string, fs fs.FS) (*restic.Node, ItemStats) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	arch := NewNewArchiver(repo, fs)
	...
	node, stats, err := arch.SaveFile(ctx, "/", filename)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewArchiverSaveFile(t *testing.T) {
	...
	for _, testfile := range tests {
		t.Run("", func(t *testing.T) {
			...
			node, stats := saveFile(t, repo, filepath.Join(tempdir, "file"), fs.Local{})
...
func TestNewArchiverSaveFileReaderFS(t *testing.T) {
	var tests = []struct {
		Data string
	}{
		{Data: ""},
...
func TestNewArchiverSaveFileIncremental(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		node, _ := saveFile(t, repo, testfile, fs.Local{})
